...
USING (
  SELECT IDTEMPS, ANNEE, MOIS, JOUR, PARTENAIRE, BENEFICIAIRE, 
         SUM(COALESCE(DEPOT, 0)) AS DEPOT, 
         SUM(ABS(COALESCE(RETRAIT, 0))) AS RETRAIT, 
         SUM(COALESCE(COMMISSION_DEPOT, 0)) AS COMMISSION_DEPOT, 
         SUM(COALESCE(COMMISSION_RETRAIT, 0)) AS COMMISSION_RETRAIT, 
         SUM(COALESCE(COMMISSION, 0)) AS COMMISSION, 
         SUM(COALESCE(REDEVANCE, 0)) AS REDEVANCE
  FROM (
    SELECT Te.IDTEMPS, Te.ANNEEC AS ANNEE, Te.MOISC AS MOIS, Te.JOUR, 
           CASE 
             WHEN UPPER(TRIM(PARTNER)) LIKE 'SERVICE ONLINE INTERNATIONAL' THEN 'SOI' 
             ELSE PARTNER 
           END AS PARTENAIRE, 
           PAYMENT_PROVIDER AS BENEFICIAIRE, 
           SUM(COALESCE(DEPOT, 0)) AS DEPOT, 
           SUM(ABS(COALESCE(RETRAIT, 0))) AS RETRAIT, 
           0.035 * SUM(COALESCE(DEPOT, 0)) AS COMMISSION_DEPOT,
           0.035 * SUM(ABS(COALESCE(RETRAIT, 0))) AS COMMISSION_RETRAIT,
           0.035 * (SUM(COALESCE(DEPOT, 0)) + SUM(ABS(COALESCE(RETRAIT, 0))) ) AS COMMISSION,
           -- Simplify case logic here if possible
           CASE 
             WHEN UPPER(TRIM(PARTNER)) IN ('1XBET', 'SERVICE ONLINE INTERNATIONAL') 
                  AND Te.JOUR >= cast('01/06/2022' as date) 
                  AND Te.JOUR < cast('01/07/2023' as date)
             THEN 0.18 * (SUM(COALESCE(DEPOT, 0)) - SUM(ABS(COALESCE(RETRAIT, 0))))
             ELSE 0 
           END AS REDEVANCE
    FROM USER_DWHPR.DTM_FRAIS_CONNEXION_AFITECH F, USER_DWHPR.DIM_TEMPS Te
    WHERE CAST(F.JOUR AS DATE) = CAST(Te.JOUR AS DATE)
      AND Te.ANNEEC = '{annee}' 
      AND Te.MOISC = '{mois}'
...
ON (R1.IDTEMPS = R2.IDTEMPS AND R1.ANNEE = R2.ANNEE AND R1.MOIS = R2.MOIS AND R1.JOUR = R2.JOUR 
    AND UPPER(TRIM(R1.PARTENAIRE)) = UPPER(TRIM(R2.PARTENAIRE)) 
    AND UPPER(TRIM(R1.BENEFICIAIRE)) = UPPER(TRIM(R2.BENEFICIAIRE)))
WHEN MATCHED THEN
  UPDATE SET R1.DEPOT = R2.DEPOT, 
             R1.RETRAIT = R2.RETRAIT, 
             R1.COMMISSION_DEPOT = R2.COMMISSION_DEPOT, 
             R1.COMMISSION_RETRAIT = R2.COMMISSION_RETRAIT, 
             R1.COMMISSION = R2.COMMISSION, 
             R1.REDEVANCE = R2.REDEVANCE
WHEN NOT MATCHED THEN
  INSERT (IDTEMPS, ANNEE, MOIS, JOUR, DEPOT, RETRAIT, COMMISSION_DEPOT, COMMISSION_RETRAIT, COMMISSION, PARTENAIRE, REDEVANCE, BENEFICIAIRE)
