QUERIES = {
    'truncate_temp': """DELETE FROM optiwaretemp.SRC_PRD_ALR_HONORE_GAMING""",
    'insert_temp': """INSERT INTO optiwaretemp.SRC_PRD_ALR_HONORE_GAMING ("REPORTDATETIME"
        ,"TICKETNUMBER" ,"STATE" ,"ISSUEDATETIME" ,"TERMINALID" ,"TERMINALDESCRIPTION" ,"RETAILCATEGORYNAME"
        ...
        ,"CANCELAGENTID" ,"CANCELAGENTNAME" ,"CANCELADMINISTRATORLOGIN" ,"MEETINGDATE" ,"MEETINGNUMBER" ,"RACENUMBER" ,"BETCATEGORYNAME"
...
WHERE idjeux IN (25,26) -- ALR et PLR Honore Gaming
    ...
    FROM (
                SELECT DISTINCT TERMINALDESCRIPTION OPERATEUR, RETAILCATEGORYNAME,
                        CASE
                           ...
                           /* WHEN TRIM(REPLACE(RETAILCATEGORYNAME,'PLR',''))LIKE 'Grand-Dakar' THEN 'Grand Dakar' -- Duplicate */
                            WHEN TRIM(REPLACE(RETAILCATEGORYNAME,'PLR',''))LIKE 'TAMBACOUNDA' THEN 'Tamba'
                            WHEN TRIM(REPLACE(RETAILCATEGORYNAME,'PLR',''))LIKE 'Richard Toll' THEN 'Tamba' -- Should be Saint-Louis?
...
SELECT '' IDLOTS, 7181 IDVENDEUR, T.IDTERMINAL, Te.IDTEMPS, J.IDJEUX, MONTANT, MONTANT_ANNULE,
...
WHERE T.OPERATEUR= F.OPERATEUR
    ...
    AND J.LIBELLEJEUX=UPPER(GAMENAME)
    AND J.LIBELLEJEUX IN ('ALR','PLR')""",
    # IPMU related (SRC_PRD_IPMU_DAILY is loaded by HonoreGaming inserter)
    'insert_main_fait_lots_ipmu': """INSERT INTO FAIT_LOTS
SELECT '' IDLOTS, 7181 IDVENDEUR, T.IDTERMINAL, Te.IDTEMPS, J.IDJEUX, MONTANT, MONTANT_ANNULE,
        NVL(PAIEMENTS,0) PAIEMENTS, TO_CHAR(Te.JOUR,'YYYY') ANNEE, TO_CHAR(Te.JOUR,'MM') MOIS, REPLACE(TO_CHAR(Te.JOUR),'/','') JOUR
    FROM (
                ...
                WHERE TO_CHAR(TO_DATE(SUBSTR(DATE_REUN,1,10)),'YYYY') = :current_year
        ) F, DIM_TERMINAL T, DIM_TEMPS Te, DIM_JEUX J
WHERE T.OPERATEUR= F.OPERATEUR
    AND T.IDSYSTEME=166
    ...
    AND J.LIBELLEJEUX=UPPER(GAMENAME)
    AND J.LIBELLEJEUX IN ('ALR','PLR')""",
    'merge_masse_commune': """MERGE INTO DTM_MASSE_COMMUNE R1 USING(
    ...
    WHERE T.OPERATEUR = F.OPERATEUR AND T.IDSYSTEME=166 AND Te.JOUR = F.DATEOP AND J.LIBELLEJEUX=UPPER(F.GAME_NAME) AND J.LIBELLEJEUX IN ('ALR','PLR')
    ...
    SELECT TO_CHAR(Te.JOUR,'YYYY') ANNEE, TO_CHAR(Te.JOUR,'MM') MOIS, Te.JOUR JOUR,
        ...
        FROM (
                    SELECT DATE_REUN DATEOP, 'IPMU TERMINAL' OPERATEUR, TO_NUMBER(TRIM(REPLACE(REPLACE(COL_EB,'.',','),' ','')))*655.95 MONTANT,
                            0  MONTANT_ANNULE, TO_NUMBER(TRIM(REPLACE(REPLACE(COL_GA,'.',','),' ','')))*655.95 PAIEMENTS,
                            1 TICKET_EMIS, 0 TICKET_ANNULE,
                            CASE WHEN TRIM(PARI)='1' THEN UPPER('SimpleGagant')
                                WHEN TRIM(PARI)='2' THEN UPPER('SimplePlace') WHEN TRIM(PARI)='3' THEN UPPER('CoupleGagant')
                                WHEN TRIM(PARI)='4' THEN UPPER('CouplePlace') WHEN TRIM(PARI)='5' THEN UPPER('CoupleOrdre')
                                WHEN TRIM(PARI)='9' THEN UPPER('Trio') WHEN TRIM(PARI)='11' THEN UPPER('Tierce')
                                WHEN TRIM(PARI)='13' THEN UPPER('QuartePlus') WHEN TRIM(PARI)='15' THEN UPPER('QuintePlus')
                                WHEN TRIM(PARI)='16' THEN UPPER('Multi') WHEN TRIM(PARI)='26' THEN UPPER('TrioOrdre')
                            END BETTYPE,
                            CASE WHEN TRIM(PARI) IN ('1','2','3','4','5','9','26')  THEN 'PLR'
                                WHEN TRIM(PARI) IN ('11','16','13','15')  THEN 'ALR'
                                ELSE 'ALR'
...
WHEN MATCHED THEN UPDATE SET R1.MONTANT=R2.MONTANT,R1.MONTANT_ANNULE=R2.MONTANT_ANNULE,R1.PAIEMENTS=R2.PAIEMENTS, R1.TICKET_EMIS=R2.TICKET_EMIS,R1.TICKET_ANNULE=R2.TICKET_ANNULE, R1.SOURCE=R2.SOURCE
...
VALUES(R2.ANNEE,R2.MOIS,R2.JOUR,R2.TERMINAL,R2.TYPE_PARI,R2.TYPE_JEU,R2.MONTANT,R2.MONTANT_ANNULE,R2.PAIEMENTS,R2.TICKET_EMIS,R2.TICKET_ANNULE,R2.SOURCE)""",
    'insert_dtm_masse_commune_ipmu': """INSERT INTO DTM_MASSE_COMMUNE_IPMU
...
FROM (
        SELECT Te.IDTEMPS, Te.ANNEEC ANNEE, Te.MOISC MOIS, S.JOUR,
                CASE WHEN UPPER(TRIM("Pari")) NOT LIKE '%SIMPLE%' THEN TO_NUMBER(REPLACE("Somme encaissées",'.')) ELSE 0 END MCI_SPORTYTOTE,
                CASE WHEN UPPER(TRIM("Pari")) LIKE '%SIMPLE%' THEN TO_NUMBER(REPLACE("Somme encaissées",'.')) ELSE 0 END MCI_SIMPLE,
                CASE WHEN UPPER(TRIM("Pari")) NOT LIKE '%SIMPLE%' THEN
                            TO_NUMBER(REPLACE("Déduction Prop sur Enjeux",'.')) + TO_NUMBER(REPLACE("Arrondis sur Rapports",'.')) + TO_NUMBER(REPLACE("CAM",'.'))
                    ELSE 0
...
GROUP BY IDTEMPS,ANNEE,MOIS,JOUR""",
    'truncate_temp_mci_ipmu_agrege': """DELETE FROM OPTIWARETEMP.SRC_PRD_MCI_IPMU_AGREGE""",
    'delete_ar_honore_gaming_prd': """DELETE FROM OPTIWARETEMP.AR_HONORE_GAMING_PRD
    WHERE TO_DATE(SUBSTR(REPORTDATETIME,1,10)) IN (SELECT DISTINCT TO_DATE(SUBSTR(REPORTDATETIME,1,10)) FROM OPTIWARETEMP.SRC_PRD_ALR_HONORE_GAMING)""",
    'insert_ar_honore_gaming_prd': """INSERT INTO OPTIWARETEMP.AR_HONORE_GAMING_PRD
    SELECT REPORTDATETIME,TICKETNUMBER,STATE,ISSUEDATETIME,TERMINALID,TERMINALDESCRIPTION,RETAILCATEGORYNAME ,RETAILCATEGORYDESCRIPTION,AGENTID,AGENTNAME,PAYMENTDATETIME,PAYMENTTERMINALID,PAYMENTTERMINALDESCRIPTION ,PAYMENTRETAILCATEGORYNAME,PAYMTRETAILCATEGORYDESCRIPTION,PAYMENTAGENTID,PAYMENTAGENTNAME,CANCELDATETIME ,CANCELTERMINALID,CANCELTERMINALDESCRIPTION,CANCELRETAILCATEGORYNAME,CANCELRTAILCATEGORYDESCRIPTION ,CANCELAGENTID,CANCELAGENTNAME,CANCELADMINISTRATORLOGIN,SUBSTR(MEETINGDATE,1,10),MEETINGNUMBER,RACENUMBER,BETCATEGORYNAME ,BETTYPE,SELECTION,MULTIVALUE,TOTALSTAKE,PAYABLEAMOUNT,PAIDAMOUNT,GAMENAME
    FROM OPTIWARETEMP.SRC_PRD_ALR_HONORE_GAMING""",
    'delete_ar_ipmu_prd': """DELETE FROM OPTIWARETEMP.AR_IPMU_PRD
    WHERE TO_DATE(SUBSTR(DATE_REUN,1,10)) IN ( SELECT DISTINCT TO_DATE(SUBSTR(DATE_REUN,1,10)) FROM OPTIWARETEMP.SRC_PRD_IPMU_DAILY )""",
    'insert_ar_ipmu_prd': """INSERT INTO OPTIWARETEMP.AR_IPMU_PRD SELECT * FROM OPTIWARETEMP.SRC_PRD_IPMU_DAILY""",
    'truncate_temp_ipmu_daily': """DELETE FROM OPTIWARETEMP.SRC_PRD_IPMU_DAILY""",
    'cleanup_temp_honore_gaming': """DELETE FROM OPTIWARETEMP.SRC_PRD_ALR_HONORE_GAMING""",
    'merge_dtm_ca_daily_plr': """MERGE INTO DTM_CA_DAILY R0 USING
(
    ...
    WHERE F.IDJEUX=J.IDJEUX AND F.IDTERMINAL=Ter.IDTERMINAL AND F.IDTEMPS=Te.IDTEMPS AND F.IDJEUX=26 AND Ter.IDSYSTEME=166
...
(
    ...
    WHERE F.IDJEUX=J.IDJEUX AND F.IDTEMPS=Te.IDTEMPS
