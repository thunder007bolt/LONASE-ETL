from utils.config_utils import get_loading_configurations
...
class Loader(ABC):
    def __init__(self, name, log_file):
        """
            ...
            Args:
                ...
                columns (list): Liste des noms des colonnes
    ...
    def check_error(self):
        """
            Vérification des fichiers d'erreur
        """
    ...
    def set_error(self, filename):
        """
            Définition d'un fichier d'erreur

            Args:
                filename (str): Nom du fichier d'erreur
        """
    ...
    def _connect_sql_server_target(self):
        ...
        self.sql_server_connexion, self.sql_server_cursor = get_db_connection(SERVER, DATABASE, USERNAME, PASSWORD,
                                                                              logger=self.logger)
    ...
    def _delete_sql_server_data(self):
        ...
        self.logger.info(f"Table SQL Server {self.sql_server_table_name} vidée.")

    def _load_sql_server_data(self, data):
        """
            Chargement des données dans la base SQL SERVER

            Args:
                data (pandas.DataFrame): Données à charger
        """
        ...
        self.logger.info(f"{len(data)} lignes chargées avec succès dans la base SQL SERVER.")

    @abstractmethod
    def _convert_file_to_dataframe(self, file):
        """
            Convertit un fichier en dataframe

            Args:
                file (Path): Chemin du fichier à convertir

            Returns:
                pandas.DataFrame: Données du fichier
        """
        pass

    def _dataframe_to_tuples(self, df):
        """
            Convertit un dataframe en tuples

            Args:
                df (pandas.DataFrame): Données à convertir
    ...
    def _delete_oracle_data(self):
        """
            Suppression des données dans la table
        """
        self.logger.info(f"Suppression des données dans la table Oracle")
        self.oracle_cursor.execute(f"TRUNCATE TABLE {self.oracle_table_name}")
        self.oracle_connexion.commit()
        self.logger.info(f"Table Oracle {self.oracle_table_name} vidée.")
    ...
    def _load_oracle_data(self, data: list):
        """
            ...
            Args:
                data (list): Liste des tuples à charger
        """
        ...
        self.oracle_connexion.commit()
        self.logger.info(f"{len(data)}  lignes chargées avec succès dans la base Oracle.")
