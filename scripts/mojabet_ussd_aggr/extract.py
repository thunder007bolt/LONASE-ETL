...
class ExtractMojabetUSSDAggr(BaseScrapper):
    def __init__(self, env_variables_list):
        ...
        super().__init__('mojabet_ussd_aggr', env_variables_list, 'logs/extract_mojabet_ussd_aggr.log',
    ...
    def _download_files(self):
        ...
        formatted_date = f"{first_day.strftime('%Y/%m/%d')} 00:00:00 - {date.strftime('%Y/%m/%d')} 23:59:59"
        ...
        for product in produit:
            ...
            while timer >= 0:
                ...
                if file is not None:
                    ...
                    df['debut'] = str(str(first_day.strftime('%d/%m/%Y')))
                    df['fin'] = str(str((date).strftime('%d/%m/%Y')))
                    df['mois'] = str(str(date.strftime('%m')))
                    df['annee'] = str(str(date.strftime('%Y')))
                    df['produit'] = product

                    newlist.append(
                        df[[
                            "debut", "fin", "mois", "annee", "Access Channel",
                            "Purchase Method", "Collection",
        ...
        filename = f"mojabet_ussd_aggr_transformed_{first_day}_{date}.csv"
