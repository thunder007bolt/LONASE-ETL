import pandas as pd, numpy as np
from base.loader2 import Loader
from utils.other_utils import load_env

load_env()


class MojabetUSSDLoad(Loader):
    def __init__(self):
        name = ('mojabet_ussd')
        log_file = 'logs/loader_mojabet_ussd.log'
        super().__init__(name, log_file)

        columns = [
            "date_mission",
            "service",
            "libelle",
            "code_ligne_budgetaire",
            # "details",
            "numcontrat",
            "montant"
        ]

        # Oracle
        self.oracle_columns = columns
        self.oracle_table_name = "OPTIWARETEMP.ENGAGEMENT_TEMP_TABLE"

        # # Sql server
        # self.sql_server_columns = columns
        # self.sql_server_table_name = "[DWHPR_TEMP].[OPTIWARETEMP].[MOJABET_USSD_DAILY]"

    def _convert_file_to_dataframe(self, file):
        df = pd.read_csv(file,index_col=False, sep=";", dtype= {"MONTANT":str})
        df["DATE"] = pd.to_datetime(df["DATE"],format="%d/%m/%Y")
        # df['Gross Payout'] = df['Gross Payout'].astype(float).round(2).astype(str)
        return df


def run_mojabet_ussd_loader():
    loader = MojabetUSSDLoad()
    loader.process_loading()


if __name__ == "__main__":
    run_mojabet_ussd_loader()
