...
class GitechLottoTransformer(GitechBaseTransformer):
    def __init__(self):
        super().__init__(
            name='gitech_lotto_ca',
            log_file='logs/transformer_gitech_lotto_ca.log',
            skiprows=5,
            date_pattern=r"date\s*:?\s*du\s*:\s*(\d{2}/\d{2}/\d{4})"
        )
    ...
    def _transform_file(self, file: Path, date=None):
        ...
        # Mapping des colonnes (spécifique à lotto_ca)
        col_mapping = {
            'S.No': 'No',
            'Agences': 'Agences',
            'Operateurs': 'Operateur',
            'Agences payer(CFA.)': 'Vente',
            'Annulation(CFA)': 'Annulation',
            'Remboursement(CFA.)': 'Remboursement',
            'Paiements(CFA.)': 'Paiement',
            'Resultats (CFA.)': 'Resultat'
        }

        # Nettoyage commun (pas de pattern spécifique pour lotto_ca)
        exclude_values = {
...
def run_gitech_lotto_ca_transformer():
    transformer = GitechLottoTransformer()
    transformer.process_transformation()

if __name__ == '__main__':
    run_gitech_lotto_ca_transformer()