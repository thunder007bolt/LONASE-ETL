...
class GitechLottoTransformer(Transformer):
    def __init__(self):
        super().__init__('gitech_lotto_ca', 'logs/transformer_gitech_lotto_ca.log')
    ...
    def extract_date_from_file(self, xlsx_file: Path) -> str:
        ...
        for idx, row in df.iterrows():
            ...
            match = re.search(r"date\s*:?\s*du\s*:\s*(\d{2}/\d{2}/\d{4})", cell_str)
            if match:
    ...
    def _transform_file(self, file: Path, date):
        ...
        try:
            ...
            data = pd.read_excel(xlsx_file, skiprows=range(0, 5))
        except Exception as e:
            self.set_error(file.name)
