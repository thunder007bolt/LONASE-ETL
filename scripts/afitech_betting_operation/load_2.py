...
class AfitechDailyPaymentActivityLoad(Loader):
    def __init__(self):
        ...
        table_name = "[DWHPR_TEMP].[OPTIWARETEMP].[SRC_AFITECH_BETTING_OPERATION_2025]"
        super().__init__(name, log_file, columns, table_name)

    def _convert_file_to_dataframe(self, file):
        df = pd.read_csv(file,sep=';',index_col=False)
        df['paid_amount'] = pd.to_numeric(df['paid_amount'].replace('-', '0'), errors='coerce').fillna(0)
        df['stake'] = pd.to_numeric(df['stake'].replace('-', '0'), errors='coerce').fillna(0)
        return df
