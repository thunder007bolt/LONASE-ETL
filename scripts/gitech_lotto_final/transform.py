...
class GitechLottoTransformer(Transformer):
    def __init__(self):
        super().__init__('gitech_lotto_final', 'logs/transformer_gitech_lotto_final.log')
    ...
    def process_transformation(self):
        ...
        while date <= self.end_date:
            ...
            pattern_lotto = Path(
                base_path + 'gitech_lotto/transformed/gitech_lotto_transformed_' + date_yyyy_mm_dd + '.csv')
            pattern_lotto_ca = Path(
                base_path + 'gitech_lotto_ca/transformed/gitech_lotto_ca_transformed_' + date_yyyy_mm_dd + '.csv')

            if pattern_lotto.exists() and pattern_lotto_ca.exists():
                import pandas as pd
                lotto_lots_df = pd.read_csv(pattern_lotto, sep=';')
                total_paiement = lotto_lots_df['Paiement'].sum()

                lotto_ca_df = pd.read_csv(pattern_lotto_ca, sep=';')
                lotto_ca_df['Paiement'] = 0

                lotto_ca_df.loc[0, 'Paiement'] = total_paiement

                name = 'gitech_lotto_final_'+date_yyyy_mm_dd+'.csv'
...
def run_gitech_lotto_final_transformer():
    transformer = GitechLottoTransformer()
    transformer.process_transformation()


if __name__ == '__main__':
    run_gitech_lotto_final_transformer()
